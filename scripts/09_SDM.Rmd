...
##### Script 09 : Models Calibration and Projection#####
...
### Inputs 
   # Environmental stack from script 04
   # Summary table for models (list_occ)
   # Spatial objects with occurrences and PsA from script 07
   # Corrected environmental stacks from script 05
   # PA table from script 07
   # CV scheme from script 08
###
...
library(tidyr)
library(tidyverse)
#library(foreach) #for parralel
#library(doParallel) for parralel
library(ggplot2)
library(tmap)
library(tmaptools)
library(biomod2)
...
envData <- readRDS(file = "../input_data/envData/Envstack.rds")

# Load Full unit list
load(file = paste0("../input_data/occurences_df/list_occ_wt_modeletype.RData"))
list_occ$code_unit[list_occ$code_unit=="Agraulis_sp.nov"]<-"Agraulis_spnov"
list_occ$code_unit[list_occ$code_unit=="Heliconius_demeter_subsp._nov."]<-"Heliconius_demeter_subspnov"



list_occ$biomodTag<-str_replace_all(list_occ$code_unit, "_", ".") #need to do that because biomod is not okay with _ in unit names

list_occ$code_unit[list_occ$code_unit=="Agraulis_spnov"]<-"Agraulis_sp.nov"
list_occ$code_unit[list_occ$code_unit=="Heliconius_demeter_subspnov"]<-"Heliconius_demeter_subsp._nov." #get back old names for extracting objects
...
#   Model_ID = rep(NA, nrow(list_occ)),              # Internal Biomod model ID
#   Start_Time = rep(NA, nrow(list_occ)),            # Starting Date & Time of last modeling
#   End_Time = rep(NA, nrow(list_occ)),              # Ending Date & Time of last modeling
#   Models_success = rep(NA, nrow(list_occ)),        # Nb of models not failing to converge
#   Models_select_0.5 = rep(NA, nrow(list_occ)),     # Nb of models selected for TSS >= 0.5
#   Mean_TSS = rep(NA, nrow(list_occ)),              # Mean TSS among all models
#   Mean_TSS_select_0.5 = rep(NA, nrow(list_occ)))   # Mean TSS among selected models for TSS >= 0.5
#                                          
# list_occ <- cbind(list_occ, New_vars_for_models_summary)
...
index_model_to_compute <- 1:nrow(list_occ)
#index_model_to_compute<-179
...
#index_model_to_compute<- c(1:15)
#index_model_to_compute<- c(16:25)

#index_model_to_compute<- 164#bug for H.charithonia churchi 118 & H. eleuchia primularis 164, solved with CV block adaptation, smaller size blocks for these two
 #index_model_to_compute<- 116
#index_model_to_compute <- c(26:146)
# index_model_to_compute <- c(147:293)
# index_model_to_compute <- c(294:nrow(list_occ))
###########################################################
#index_model_to_compute <- 10
...
p<-progress_estimated(length(index_model_to_compute), min_time=60)
...
p$pause(0.1)$tick()$print()
  ...
  # Load unit/OMU name
  unit <- as.character(list_occ$code_unit[i]) 
  model_type <- list_occ$initial_model_type[i]
  BMunit<-as.character(list_occ$biomodTag[i])
  
  if (model_type == "rasterized") {
    list_occ$Start_Time[list_occ$code_unit == unit] <- as.character(Sys.time())
    ...
    list_occ$End_Time[list_occ$code_unit == unit] <- as.character(Sys.time())
    ...
    # Unparallelized version
     list_occ$Start_Time[list_occ$code_unit == unit] <- as.character(Sys.time()) # Store date and time of starting of last modeling process
    ...
    # if(row.names(unit_points.PsA[1,]!=1)){row.names()}
    
    # Load environmental stack with corrections for extrapoled data points
    load(file = paste0("../input_data/Species_data/Env_Stacks/Env_stack_",unit,".RData"))
    unit_env
    ...
    if(!dir.exists(paste0("../models/", BMunit))) { # Test if the folder exists already or not
      dir.create(paste0("../models/", BMunit), recursive = T) # Create folder if absent
    }
    ...
    formated.input.data <- BIOMOD_FormatingData(resp.var = unit_points.PsA,    # Spatial object with occurrences and PsA
                                                expl.var = unit_env,           # Env stack
                                                resp.name = BMunit,              # Unit
                                                PA.nb.rep = ncol(PA.table),    # Nb of PsA sets
                                                PA.nb.absences = sum(unit_points.PsA@data, na.rm = T), # Nb of occurrences = Nb of PsA per set
                                                PA.strategy = "user.defined",  # To allow to provide a custom PA_table
                                                PA.table = PA.table)           # Provide the custom PA_table
    
    save(formated.input.data, file = paste0(initial.wd,"/models/",BMunit,"/formated.input.data.RData"))
    saveRDS(formated.input.data, file = paste0(initial.wd,"/models/",BMunit,"/formated.input.data.rds"))
    ...
    if (model_type == "complete") { # Modeling for OMU with enough obs (N.obs ≥ 30) # 3 PsA sets # Spatial CV with 3 folds 
      ...
      calib_array <- readRDS(file = paste0("../input_data/Species_data/CV_blocks/calib_arrays/calib_array_", unit,".rds"))
      ...
      model.runs <- BIOMOD_Modeling(data = formated.input.data,
                                    ...
                                    VarImport = 10, # Nb of data permutation to evaluate variable importance
    ...
    save(model.runs, file = paste0(initial.wd,"/models/",BMunit,"/model.runs.RData")) 
    saveRDS(model.runs, file = paste0(initial.wd,"/models/",BMunit,"/model.runs.rds"))
    ...
    # # Store infos from modeling results: Unparallelized version
     list_occ$Model_ID[list_occ$code_unit == unit] <- model.runs@modeling.id # Keep model ID
     list_occ$Models_success[list_occ$code_unit == unit] <- length(get_built_models(model.runs)) # Save nb of converging models
    ...
    if (!(length(get_built_models(model.runs)) > 0)) { # Stop process if no model has converged
      ...
      list_occ$Mean_TSS_full[list_occ$code_unit == unit] <- round(mean(eval.runs$value, na.rm = T),3) # Mean value for TSS among all converged models
      list_occ$Models_select_TSS_0.5[list_occ$code_unit == unit] <- sum(eval.runs$value >= 0.5, na.rm = T) # Nb of model with TSS ≥ 0.5
      list_occ$Mean_TSS_select_TSS_0.5[list_occ$code_unit == unit] <- round(mean(eval.runs$value[eval.runs$value >= 0.5], na.rm = T),3) # Mean value for TSS among all models with TSS ≥ 0.5
      ...
      save(projection.runs, file = paste0(initial.wd,"/models/",BMunit,"/proj_Current/projection.runs.RData"))
      saveRDS(projection.runs, file = paste0(initial.wd,"/models/",BMunit,"/proj_Current/projection.runs.rds"))
      
      # Reload .RData and Save final stack under .rds to be able to load it in any named object
      load(file = paste0(initial.wd,"/models/",BMunit,"/proj_Current/proj_Current_",BMunit,".RData")) 
      # load(projection.runs@proj@link) # Does the same but retrieve the path to the .RData from the projection.runs object
      saveRDS(eval(parse(text = paste0("proj_Current_",BMunit))), file = paste0(initial.wd,"/models/",BMunit,"/proj_Current/proj_Current_",BMunit,".rds"))
      rm(list = paste0("proj_Current_",BMunit)) # Remove it from the env.
      
      # Reload .RData and Save Clamp mask under .rds to be able to load it in any named object
      load(file = paste0(initial.wd,"/models/",BMunit,"/proj_Current/proj_Current_ClampingMask.RData")) 
      saveRDS(eval(parse(text = paste0("proj_Current_",BMunit,"_ClampingMask"))), file = paste0(initial.wd,"/models/",BMunit,"/proj_Current/proj_Current_",BMunit,"_ClampingMask.rds"))
      rm(list = paste0("proj_Current_",BMunit,"_ClampingMask")) # Remove it from the env.
      ...
      rm(list = c("eval.runs", "formated.input.data", "model.runs", "unit_env", "unit_points.PsA"))
  ...
  save(list_occ, file = paste0(initial.wd,"/models/0_list_occ_outputs/list_occ_",min(index_model_to_compute),"_to_",max(index_model_to_compute),".RData")) 
  saveRDS(list_occ, file = paste0(initial.wd,"/models/0_list_occ_outputs/list_occ_",min(index_model_to_compute),"_to_",max(index_model_to_compute),".rds"))
...
save(list_occ, file = paste0(initial.wd,"/models/0_list_occ_outputs/list_occ_",min(index_model_to_compute),"_to_",max(index_model_to_compute),".RData")) 
saveRDS(list_occ, file = paste0(initial.wd,"/models/0_list_occ_outputs/list_occ_",min(index_model_to_compute),"_to_",max(index_model_to_compute),".rds"))
# save(list_occ, file = "../input_data/list_occ.RData")


# Parallelized version

# ### Manage outputs from the parallelized runs and save them in the summary table: list_occ
# 
...
# 
# # Save the info in case the addition into list_occ fails or running on the cluster (not a good idea to reand/write the same list.model object in all instances)
...
#   list_occ$Start_Time[temp$index[i]] <- temp$Start_Time[i]
#   list_occ$End_Time[temp$index[i]] <- temp$End_Time[i]
#   list_occ$Model_ID[temp$index[i]] <- temp$Model_ID[i]
#   list_occ$Models_success[temp$index[i]] <- temp$Models_success[i]
#   list_occ$Mean_TSS_full[temp$index[i]] <- temp$Mean_TSS_full[i]
#   list_occ$Models_select_TSS_0.5[temp$index[i]] <- temp$Models_select_TSS_0.5[i]
#   list_occ$Mean_TSS_select_TSS_0.5[temp$index[i]] <- temp$Mean_TSS_select_TSS_0.5[i]
# }
# 
# View(list_occ)
...
# Choice of unit
load(file = paste0(initial.wd,"../input_data/occurences_df/list_occ_after_modeling.RData"))
modeling.wd <- paste0(initial.wd, "/models/")
occ2<-list_occ[list_occ$initial_model_type!="rasterized",]

p <- progress_estimated(nrow(occ2), min_time = 30)

for( i in 1:nrow(occ2)) 
 { p$pause(0.1)$tick()$print()

  unit <- occ2$code_unit[i]
  BMunit <-occ2$biomodTag[i]
# Loading occurrence data
load(paste0(initial.wd,"/input_data/Species_data/Spatial_Points_Objects/occurrences_", unit,".RData")) #



# Loading projection stack
proj_stack <- readRDS(file = paste0(modeling.wd,BMunit,"/proj_Current/proj_Current_",BMunit,".rds"))


tiff(file=paste0("../Maps/Stack_models/Modele_stack_",unit,".tiff"), units="in", width=5, height=5, res=150)

plot(proj_stack[[sample(length(proj_stack@layers),9, replace=FALSE)]], cex.main=0.65) #9 random models map (out of 27 more or less) for each unit
points(unit_points2, pch = 16)

dev.off()

# Loading Clamping mask

Clamping_mask <- readRDS(file = paste0(initial.wd,"/models/",BMunit,"/proj_Current/proj_Current_",BMunit,"_ClampingMask.rds"))
tiff(file=paste0("../Maps/Clamping_masks/Clamping_mask",unit,".tiff"), units="in", width=5, height=5, res=100)

plot(Clamping_mask)
points(x=unit_points2$x, y=unit_points2$y, pch = 16, cex=0.1)
dev.off()
#print(paste0("Stack and clamping masks plot ok for ",unit," ", i, "/", nrow(occ2)))
}
#
######  
```
...
```{r}
##### Explore model projection outputs with algo type
...
tiff(file=paste0("../Maps/Models_algo_and_occurrences/Modele_stack_",unit,".tiff"), units="in", width=5, height=2.5, res=300)

par(mfrow=c(1,3))
plot(proj_stack[[1]], cex.main=0.7, reset=F, main=names(proj_stack[[1]]))#3 models map (one for each algo) for each unit
points(x=unit_points2$x, y=unit_points2$y, pch = 1, cex=0.05)
plot(proj_stack[[2]], cex.main=0.7, reset=F, , main=names(proj_stack[[2]]))
points(x=unit_points2$x, y=unit_points2$y, pch = 1, cex=0.05)
plot(proj_stack[[3]], cex.main=0.7, , main=names(proj_stack[[3]]))
points(x=unit_points2$x, y=unit_points2$y, pch = 1, cex=0.05,)
