...
##### Script 11:  Manual evaluation of sub.models #####
...
# Load exemple units to use as template to create the empty array that will gather the evaluation data for all units

# Load Summary table for unut models
...
unit_complete <- as.character(modeled_unit$biomodTag[which.max(modeled_unit$initial_model_type == "complete")])
unit_restricted <- as.character(modeled_unit$biomodTag[which.max(modeled_unit$initial_model_type == "restricted")])

model.runs_complete <- readRDS(file = paste0("../models/",unit_complete,"/model.runs.rds"))
model.runs_restricted <- readRDS(file = paste0("../models/",unit_restricted,"/model.runs.rds"))
...
all.units_evaluation <- array(dim = c(nrow(modeled_unit),  # Nb of units/OMUs
                              ...
                              dimnames = list(unit = modeled_unit$biomodTag,   # Unit = OMU
...
# Load Summary table for unit models
load(file = paste0("../input_data/occurences_df/list_occ_after_modeling.RData"))
modeled_unit <- list_occ[!is.na(list_occ$Model_ID), ]
...
for (i in 1:nrow(modeled_unit)) 
{
 p$pause(0.1)$tick()$print() 
  ...
  model_type <- modeled_unit$initial_model_type[i]
  
  cat(paste0("\n", Sys.time()," ------ Evaluation starts for ", unit, " = Unit N°",i,"/",nrow(modeled_unit)," ------\n"))
  ...
  save(array_evals, file = paste0("../controls/evaluations/By_unit/eval_results/array_evals_",unit,".RData"), version = "2")
  saveRDS(array_evals, file = paste0("../controls/evaluations/By_unit/eval_results/array_evals_",unit,".rds"), version = "2")
  save(array_cutoffs, file = paste0("../controls/evaluations/By_unit/eval_results/array_cutoffs_",unit,".RData"), version = "2")
  saveRDS(array_cutoffs, file = paste0("../controls/evaluations/By_unit/eval_results/array_cutoffs_",unit,".rds"), version = "2")
  
  # Store cut-off test infos for this unit, for reproductibility
  save(jaccard.test.list, file = paste0("../controls/evaluations/By_unit/test_data/jaccard.tests_",unit,".RData"), version = "2")
  saveRDS(jaccard.test.list, file = paste0("../controls/evaluations/By_unit/test_data/jaccard.tests_",unit,".rds"), version = "2")
  save(TSS.test.list, file = paste0("../controls/evaluations/By_unit/test_data/TSS.tests_",unit,".RData"), version = "2")
  saveRDS(TSS.test.list, file = paste0("../controls/evaluations/By_unit/test_data/TSS.tests_",unit,".rds"), version = "2")
  
  cat(paste0("\n", Sys.time()," ------ Evaluation done for ", unit, " = Unit N°",i,"/",nrow(modeled_unit)," ------\n"))
...
### Explore results
# i <- 2
# unit <- as.character(modeled_unit$Tag.model[i])
# 
# load(file = paste0("../../controls/evaluations/By_unit/test_data/TSS.tests_",unit,".RData"))
# load(file = paste0("../../controls/evaluations/By_unit/eval_results/array_evals_",unit,".RData"))
###
...
pdf(file = "../controls/evaluations/Eval_Jaccard_boxplot.pdf", height = 6, width = 10) 
...
for (i in 1:nrow(modeled_unit))
{
 p$pause(0.1)$tick()$print()
  ...
  pdf(file = paste0("../controls/evaluations/By_unit/boxplots/Jaccard/Preselect_Eval_Jaccard_boxplot_",unit,".pdf"), height = 6, width = 10)
  ...
  pdf(file = paste0("../controls/evaluations/By_unit/boxplots/TSS/Preselect_Eval_TSS_boxplot_",unit,".pdf"), height = 6, width = 10)
